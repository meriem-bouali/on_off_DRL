...
class TrackDensity:
    ...
    def get_edge_density(self):  # get density in pc/mi/ln
        edges_length, edges_nb_lanes = self.get_main_edge_length()
        dt = datetime.now().strftime("%d-%m-%Y")  # "%d-%m-%Y_(%H-%M)"
        traci.start(
            [
                "sumo",
                "-n",
        ...
        file_pth = os.path.join(FP.sim_validator_pth, f"density_{dt}.csv")
            ...
            while i <= (SC.ego_veh_entry_step + EC.episode_max_length):
                ...
                if i % self.freq == 0:
                    ...
                    for edge_id in self.main_edges_set:
                        # density pc/m
                        density_m = traci.edge.getLastStepVehicleNumber(edge_id) / edges_length[edge_id]

                        # pc/mi
                        density_mi = self.PCperM_to_PCperMi(density_m)

                        # pc/mi/ln
                        density_mi_ln = density_mi / edges_nb_lanes[edge_id]
...
if __name__ == "__main__":
    ...
    track_density.get_edge_density()
