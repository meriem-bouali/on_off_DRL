...
class BCQAgent(AgentOffMixin, metaclass=CustomABCMeta):
    ...
    def __init__(
        ...
        self.target_network = BCNetwork(self.device, self.lr, self.nn_conf_func, self.input_dim, self.output_dim)
    ...
    def learn(self, transitions):
        ...
        with T.no_grad():
            ...
            # BCQ constraint: filter actions below threshold
            imt = (imt / imt.max(1, keepdim=True)[0] > self.threshold).float()

            # Select next action using online network (Double DQN style)
            # Mask out actions not supported by behavioral policy
